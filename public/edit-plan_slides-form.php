...
if (isset($_POST['btnUpdate'])) {
    ...
    $sql = "UPDATE plan_slides SET name='$name',link='$link' WHERE id = '$ID'";
    $db->sql($sql);
    $result = $db->getResult();
    if (!empty($result)) {
        $error['update_slide'] = " <span class='label label-danger'>Failed</span>";
    } else {
        $error['update_slide'] = " <span class='label label-success'>Plan slide Updated Successfully</span>";
    }
    ...
    if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
        ...
        $sql = "UPDATE plan_slides SET `image`='$upload_image' WHERE `id`='$ID'";
        $db->sql($sql);
        ...
        if ($update_result == 1) {
            $error['update_slide'] = " <section class='content-header'><span class='label label-success'>Plan slide updated Successfully</span></section>";
        } else {
...
$sql_query = "SELECT * FROM `plan_slides` WHERE id = '$ID'";
$db->sql($sql_query);
$res = $db->getResult();
?>

<section class="content-header">
    <h1>Edit Plan Slide <small><a href='plan_slides.php'> <i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Plan Slide</a></small></h1>
